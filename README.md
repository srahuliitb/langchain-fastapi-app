...
- **Ollama** (required for `/query`): [ollama.ai](https://ollama.ai). Start the app or run `ollama serve`, then pull your model: `ollama pull qwen2.5:7b` (or match `OLLAMA_MODEL`). If you get 503 on `/query`, Ollama is not running or the model is not pulled.
